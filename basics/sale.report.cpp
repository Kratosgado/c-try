...
This uses STL vectors instead of arrays.
*/
#include <iostream>
#include <iomanip>
#include <vector>
...
int main() {
   ...
   // must provide an initialization routing.
   initVectors(id, units, prices);

   // calculate each product's sales;
   calcSales(units, prices, sales);

   // sort the elements in the sales array in descending order and shuffle the Id
   // numbers in the id array to keep them in parallel
   dualSort(id, sales);
...
void initVectors(vector<int>& id, vector<int>& units, vector<double>& prices) {
   // initialize the id vector with the ID numbers 914 throw 922
